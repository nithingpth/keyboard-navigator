...
function inDeviationRange(fromNodeOrdinate,activeElementOrdinate,deviation){
...
keyBoardNavigator.navigationHandler = function(event){
    if(!this.pause)
    {
        ...
        //tab or shift tab
        if(event.keyCode == keys["tab"] && this.listenOnTabKey){
            if(this.customTabLogic == false){
                ...
                if(this.additionalCustomTabLogic && typeof(this.additionalCustomTabLogic) == "function"){
                    executeDefaultTabbingLogic = this.additionalCustomTabLogic.call(this);                    
                }
