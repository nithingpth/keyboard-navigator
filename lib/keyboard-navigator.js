...
keyBoardNavigator.persistFocus = function(){
    if(document.activeElement && document.activeElement != document.body && this.pause == false){
        
        var prevActiveElementXPath = this.getXPathForElement(document.activeElement);           
...
keyBoardNavigator.inDeviationRange = function(fromNodeOrdinate,activeElementOrdinate,deviation){
    return this.inRange(activeElementOrdinate,fromNodeOrdinate-deviation,fromNodeOrdinate+deviation)
}
...
keyBoardNavigator.queue = class Queue{
    ...
    getDocumentContainingNodeFromLast = function(){
        ...
        for(var i=this.items.length-1;i>-1;i--){
            var node = keyBoardNavigator.getElementByXPath(this.items[i])
            ...
            if(this.useDefaultFallbackFocusLogic){
                ...
                setTimeout(()=>{
                    ...
                    if(!isClickedElementStillExists){
                        ...
                        isClickedElementStillExists.focus();
                    }
                  },0)     
            }
        ...
        if([this.keys["up"],this.keys["down"],this.keys["left"],this.keys["right"]].includes(event.keyCode)){
            ...
            if(arrowNavigableElements.length>0){
                if(isContainingOrTiggeringElement){
                    ...
                    if(nextArrowNavigableElementToFocus){
                        nextArrowNavigableElementToFocus.focus();
                        event.stopPropagation();
                        event.preventDefault();
                    }
                }
...
try{
    ...
    console.log("from keboard-navigator.js: module is not defined",e);
}
